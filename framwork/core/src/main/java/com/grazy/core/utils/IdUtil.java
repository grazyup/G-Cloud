...
public class IdUtil {
    ...
    public synchronized static Long get() {
        ...
        if (timestamp < lastTimestamp) {
            ...
            throw new GCloudBusinessException(String.format("Clock moved backwards.  Refusing to generate id for %d milliseconds", lastTimestamp - timestamp));
        }
    ...
    public static Long decrypt(String decryptId) {
        if (StringUtils.isNotBlank(decryptId)) {
            ...
            throw new GCloudBusinessException("AES128Util.aesDecode fail");
        }
        throw new GCloudBusinessException("the decryptId can not be empty");
    }
    ...
    public static List<Long> decryptIdList(String decryptIdStr) {
        ...
        List<String> decryptIdList = Splitter.on(GCloudConstants.COMMON_SEPARATOR).splitToList(decryptIdStr);
