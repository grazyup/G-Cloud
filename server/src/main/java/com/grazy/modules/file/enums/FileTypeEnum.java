...
/**
 * @Author: grazy
 * @Date: 2024-02-29 21:15
 * @Description: 文件类型枚举 (文件类型 -- 1 普通文件 2 压缩文件 3 excel 4 word 5 pdf 6 txt 7 图片 8 音频 9 视频 10 ppt 11 源码文件 12 csv )
 */
...
public enum FileTypeEnum {
     ...
     *
     * @param fileSuffix 文件后置
     * @return 文件类型的code
     */
    public static Integer getFileTypeCode(String fileSuffix){
        Optional<FileTypeEnum> first = Arrays.stream(values())
                .sorted(Comparator.comparingInt(FileTypeEnum::getCode).reversed())
                .filter(value -> value.getTester().test(fileSuffix))
                .findFirst();
        if(first.isPresent()){
            return first.get().getCode();
        }
        throw new GCloudBusinessException("获取文件类型失败");
    }
}
